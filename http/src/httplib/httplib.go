...
var (
	ErrCertMismatch = errors.New("cert does not match pinned fingerprint")
)
...
func PinnedClient(pinnedCert string, skipCAVerification bool) Client {
	client := &http.Client{
		Timeout: time.Second * 10}

	client.Transport = &http.Transport{
		DialTLS:             makePinnedDialer(pinnedCert, skipCAVerification),
